...
def non_conv_experiment(eta, file='../data/opt_omega'):
    print(eta)
...
def plot_non_conv(file='../data/opt_omega.npz', num_runs=20):
    arrs  = np.load(file)
...
def parallel_non_conv(etas):
    with concurrent.futures.ProcessPoolExecutor() as executor:
        future_f = [executor.submit(non_conv_experiment, eta, '../data/opt_omega') for eta in etas]
